...
def eval_h_tricky(x,lagrange,obj_factor,flag,Rf_C):  
    ...
    cdef double[:, :] Hessian_Py = np.zeros((T,T))
    cdef double[:] x_C = np.zeros((T))
    cdef double[:, :] data_C = np.zeros((N,M))
    x_C = x
    data_C = data
...
#        for ii in range(nnzh_C):
#            Hessian_Py[row_final_C[ii]][col_final_C[ii]] = Hessian[row_final_C[ii]][col_final_C[ii]]
                    
#        for i in range(T):
#            for j in range(T):
#                Hessian_Py[i][j] = Hessian[i][j]     
#                    
        return np.asarray(obj_factor*np.asarray(Hessian_Py))